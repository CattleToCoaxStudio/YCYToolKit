...
#import "NSData+YCYGzip.h"
#import <zlib.h>
#import <dlfcn.h>

@implementation NSData (YCYGzip)

static void *ycy_libzOpen()
{
    static void *libz;
...
- (NSData *)ycy_gzippedDataWithCompressionLevel:(float)level
{
    if (self.length == 0 || [self ycy_isGzippedData])
    {
        return self;
    }
    
    void *libz = ycy_libzOpen();
...
- (NSData *)ycy_gzippedData
{
    return [self ycy_gzippedDataWithCompressionLevel:-1.0f];
}

- (NSData *)ycy_gunzippedData
{
    if (self.length == 0 || ![self ycy_isGzippedData])
    {
        return self;
