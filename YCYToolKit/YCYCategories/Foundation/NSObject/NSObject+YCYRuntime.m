//
//  NSObject+YCYRuntime.m
//
//  Created by Jakey on 14/12/30.
//  Copyright (c) 2014年 www.skyfox.org. All rights reserved.
//

#import "NSObject+YCYRuntime.h"
#import <objc/runtime.h>
BOOL ycy_method_swizzle(Class klass, SEL origSel, SEL altSel)
{
    if (!klass)
        return NO;
...
void ycy_method_append(Class toClass, Class fromClass, SEL selector)
{
...
void ycy_method_replace(Class toClass, Class fromClass, SEL selector)
{
    if (!toClass || !fromClass || ! selector)
        return;
...
@implementation NSObject (ycyRuntime)

+ (void)ycy_swizzleMethod:(SEL)originalMethod withMethod:(SEL)newMethod
{
    ycy_method_swizzle(self.class, originalMethod, newMethod);
}

+ (void)ycy_appendMethod:(SEL)newMethod fromClass:(Class)klass
{
    ycy_method_append(self.class, klass, newMethod);
}

+ (void)ycy_replaceMethod:(SEL)method fromClass:(Class)klass
{
    ycy_method_replace(self.class, klass, method);
}

- (BOOL)ycy_respondsToSelector:(SEL)selector untilClass:(Class)stopClass
{
    return [self.class ycy_instancesRespondToSelector:selector untilClass:stopClass];
}

- (BOOL)ycy_superRespondsToSelector:(SEL)selector
{
    return [self.superclass instancesRespondToSelector:selector];
}

- (BOOL)ycy_superRespondsToSelector:(SEL)selector untilClass:(Class)stopClass
{
    return [self.superclass ycy_instancesRespondToSelector:selector untilClass:stopClass];
}

+ (BOOL)ycy_instancesRespondToSelector:(SEL)selector untilClass:(Class)stopClass
{
