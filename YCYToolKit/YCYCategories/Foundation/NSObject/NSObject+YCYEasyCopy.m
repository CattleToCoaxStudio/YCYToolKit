...
#import "NSObject+YCYEasyCopy.h"
#import <objc/runtime.h>

@implementation NSObject (YCYEasyCopy)
    ...
    while (instanceClass != [NSObject class]) {
        ...
        for (int i = 0; i < propertyListCount; i++) {
            ...
            if (propertyName && !readonly) {
                ...
                BOOL flag = [NSObject ycy_isNSObjectClass:propertyValueClass];
                if (flag) {
                    if ([propertyValue conformsToProtocol:@protocol(NSCopying)]) {
                        ...
                        [copyValue ycy_easyDeepCopy:propertyValue];
                        [self setValue:copyValue forKey:propertyName];
                    }
                }else{
                    [self setValue:propertyValue forKey:propertyName];
...
+ (BOOL)ycy_isNSObjectClass:(Class)clazz{
    ...
    if (flag) {
        ...
        if (!superClass) {
            ...
            return  [NSObject ycy_isNSObjectClass:superClass];
        }
    }
}
@end
