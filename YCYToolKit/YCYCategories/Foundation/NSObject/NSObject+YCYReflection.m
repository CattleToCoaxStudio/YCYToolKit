//
//  NSObject+YCYReflection.m
//
//  Created by Jakey on 15/12/22.
//  Copyright © 2015年 Jakey. All rights reserved.
//

#import "NSObject+YCYReflection.h"
#import <objc/runtime.h>

@implementation NSObject (YCYReflection)
...
{
    return [[self class] ycy_propertyKeys];
}
+ (NSArray *)ycy_propertyKeys {
...
{
    return [[self class] ycy_propertiesInfo];
}
/**
...
+ (NSArray *)ycy_propertiesInfo
{
    ...
    for (int i = 0; i < propertyCount; i++)
    {
        [propertieArray addObject:({
            ...
            NSDictionary *dictionary = [self ycy_dictionaryWithProperty:properties[i]];
...
+ (NSArray *)ycy_propertiesWithCodeFormat
{
    ...
    NSArray *properties = [[self class] ycy_propertiesInfo];
...
//            NSString *argString = [NSString stringWithCString:arg encoding:NSUTF8StringEncoding];
            [arguments addObject:[[self class] ycy_decodeType:arg]];
        }
        
        NSString *returnTypeString =[[self class] ycy_decodeType:returnType];
        NSString *encodeString = [[self class] ycy_decodeType:encoding];
...
-(NSDictionary *)ycy_protocolList{
    return [[self class]ycy_protocolList];
}
+ (NSDictionary *)ycy_protocolList
{
...
+ (NSArray *)ycy_instanceVariable
{
    ...
    NSMutableArray *result = [NSMutableArray array];
    for (int i = 0; i < outCount; i++) {
        NSString *type = [[self class] ycy_decodeType:ivar_getTypeEncoding(ivars[i])];
...
#pragma mark -- help
+ (NSDictionary *)ycy_dictionaryWithProperty:(objc_property_t)property
{
...
+ (NSString *)ycy_decodeType:(const char *)cString
{
    ...
    } else
    {
        if ([[result substringToIndex:1] isEqualToString:@"^"]) {
            result = [NSString stringWithFormat:@"%@ *",
                      [NSString ycy_decodeType:[[result substringFromIndex:1] cStringUsingEncoding:NSUTF8StringEncoding]]];
        }
    }
    return result;
}
