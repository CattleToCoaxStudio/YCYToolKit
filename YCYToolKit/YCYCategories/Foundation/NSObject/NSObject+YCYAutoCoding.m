...
//
#import "NSObject+YCYAutoCoding.h"
#import <objc/runtime.h>
#pragma GCC diagnostic ignored "-Wgnu"
static NSString *const YCYAutocodingException = @"ycyAutocodingException";
@implementation NSObject (YCYAutoCoding)
+ (BOOL)supportsSecureCoding
{
    return YES;
}
+ (instancetype)ycy_objectWithContentsOfFile:(NSString *)filePath
{
    //load the file
...
- (BOOL)ycy_writeToFile:(NSString *)filePath atomically:(BOOL)useAuxiliaryFile
{
...
+ (NSDictionary *)ycy_codableProperties
{
    //deprecated
...
- (NSDictionary *)ycy_codableProperties
{
    ...
    if (!codableProperties)
    {
        ...
        {
            [(NSMutableDictionary *)codableProperties addEntriesFromDictionary:[subclass ycy_codableProperties]];
...
- (NSDictionary *)ycy_dictionaryRepresentation
{
    NSMutableDictionary *dict = [NSMutableDictionary dictionary];
    for (__unsafe_unretained NSString *key in [self ycy_codableProperties])
    {
...
- (void)ycy_setWithCoder:(NSCoder *)aDecoder
{
    BOOL secureAvailable = [aDecoder respondsToSelector:@selector(decodeObjectOfClass:forKey:)];
    BOOL secureSupported = [[self class] supportsSecureCoding];
    NSDictionary *properties = [self ycy_codableProperties];
    for (NSString *key in properties)
    {
        ...
        if (object)
        {
            ...
            {
                [NSException raise:YCYAutocodingException format:@"Expected '%@' to be a %@, but was actually a %@", key, propertyClass, [object class]];
            }
...
{
    [self ycy_setWithCoder:aDecoder];
    return self;
}
#pragma clang diagnostic pop
- (void)encodeWithCoder:(NSCoder *)aCoder
{
    for (NSString *key in [self ycy_codableProperties])
    {
