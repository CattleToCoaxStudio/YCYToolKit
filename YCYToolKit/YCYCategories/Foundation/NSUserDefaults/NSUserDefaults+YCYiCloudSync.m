//
//  NSUserDefaults+YCYiCloudSync.m
//  Category
//
...
#import "NSUserDefaults+YCYiCloudSync.h"

@implementation NSUserDefaults (YCYiCloudSync)
...
- (void)ycy_removeValueForKey:(NSString *)key iCloudSync:(BOOL)sync
{
    [self ycy_removeObjectForKey:key iCloudSync:sync];
}



-(void)ycy_setObject:(id)value forKey:(NSString *)defaultName iCloudSync:(BOOL)sync
{
    if (sync)
...
- (void)ycy_removeObjectForKey:(NSString *)key iCloudSync:(BOOL)sync
{
    if (sync)
