...
#import "NSDate+YCYInternetDateTime.h"
...
@implementation NSDate (YCYInternetDateTime)

// Instantiate single date formatter
+ (NSDateFormatter *)ycy_internetDateTimeFormatter {
    @synchronized(self) {
...
+ (NSDate *)ycy_dateFromInternetDateTimeString:(NSString *)dateString formatHint:(DateFormatHint)hint {
     // Keep dateString around a while (for thread-safety)
	NSDate *date = nil;
    if (dateString) {
        if (hint != DateFormatHintRFC3339) {
            // Try RFC822 first
            date = [NSDate ycy_dateFromRFC822String:dateString];
            if (!date) date = [NSDate ycy_dateFromRFC3339String:dateString];
        } else {
            // Try RFC3339 first
            date = [NSDate ycy_dateFromRFC3339String:dateString];
            if (!date) date = [NSDate ycy_dateFromRFC822String:dateString];
        }
    }
...
+ (NSDate *)ycy_dateFromRFC822String:(NSString *)dateString {
     // Keep dateString around a while (for thread-safety)
    NSDate *date = nil;
    if (dateString) {
        NSDateFormatter *dateFormatter = [NSDate ycy_internetDateTimeFormatter];
...
+ (NSDate *)ycy_dateFromRFC3339String:(NSString *)dateString {
     // Keep dateString around a while (for thread-safety)
    NSDate *date = nil;
    if (dateString) {
        NSDateFormatter *dateFormatter = [NSDate ycy_internetDateTimeFormatter];
