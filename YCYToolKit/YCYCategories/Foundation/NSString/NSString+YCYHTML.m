...
static YCY_HTMLEscapeMap ycy_gAsciiHTMLEscapeMap[] = {
...
static YCY_HTMLEscapeMap ycy_gUnicodeHTMLEscapeMap[] = {
...
static int YCY_EscapeMapCompare(const void *ucharVoid, const void *mapVoid) {
	const unichar *uchar = (const unichar*)ucharVoid;
	const YCY_HTMLEscapeMap *map = (const YCY_HTMLEscapeMap*)mapVoid;
	int val;
	if (*uchar > map->uchar) {
		val = 1;
...
@implementation NSString (ycyHTML)
#pragma --mark GTM method

- (NSString *)ycy_stringByEscapingHTMLUsingTable:(YCY_HTMLEscapeMap*)table
...
//		_GTMDevLog(@"Unable to allocate buffer or data2");
	...
	for (NSUInteger i = 0; i < length; ++i) {
		YCY_HTMLEscapeMap *val = bsearch(&buffer[i], table,
									 size / sizeof(YCY_HTMLEscapeMap),
									 sizeof(YCY_HTMLEscapeMap), YCY_EscapeMapCompare);
...
- (NSString *)ycy_stringByEscapingForHTML {
	return [self ycy_stringByEscapingHTMLUsingTable:ycy_gUnicodeHTMLEscapeMap
											 ofSize:sizeof(ycy_gUnicodeHTMLEscapeMap) 
									escapingUnicode:NO];
} // gtm_stringByEscapingHTML

- (NSString *)ycy_stringByEscapingForAsciiHTML {
	return [self ycy_stringByEscapingHTMLUsingTable:ycy_gAsciiHTMLEscapeMap
											 ofSize:sizeof(ycy_gAsciiHTMLEscapeMap) 
									escapingUnicode:YES];
} // gtm_stringByEscapingAsciiHTML

- (NSString *)ycy_stringByUnescapingFromHTML {
	...
	do {
		...
		if (length > 3 && length < 11) {
			if ([escapeString characterAtIndex:1] == '#') {
				...
				// "standard" sequences
				for (unsigned i = 0; i < sizeof(ycy_gAsciiHTMLEscapeMap) / sizeof(YCY_HTMLEscapeMap); ++i) {
					if ([escapeString isEqualToString:ycy_gAsciiHTMLEscapeMap[i].escapeSequence]) {
						[finalString replaceCharactersInRange:escapeRange withString:[NSString stringWithCharacters:&ycy_gAsciiHTMLEscapeMap[i].uchar length:1]];
						break;
					}
				}
			}
...
- (NSString *)ycy_stringWithNewLinesAsBRs {
    @autoreleasepool {
        // Strange New lines:
        //	Next Line, U+0085
        //	Form Feed, U+000C
...
- (NSString *)ycy_stringByRemovingNewLinesAndWhitespace {
    @autoreleasepool {
...
- (NSString *)ycy_stringByLinkifyingURLs {
...
- (NSString *)ycy_stringByStrippingTags {
    @autoreleasepool {
        ...
        finalString = [result ycy_stringByRemovingNewLinesAndWhitespace];
...
- (NSString *)ycy_stringByConvertingHTMLToPlainText {
    @autoreleasepool {
        // Character sets
        NSCharacterSet *stopCharacters = [NSCharacterSet characterSetWithCharactersInString:[NSString stringWithFormat:@"< \t\n\r%C%C%C%C", (unichar)0x0085, (unichar)0x000C, (unichar)0x2028, (unichar)0x2029]];
        NSCharacterSet *newLineAndWhitespaceCharacters = [NSCharacterSet characterSetWithCharactersInString:[NSString stringWithFormat:@" \t\n\r%C%C%C%C", (unichar)0x0085, (unichar)0x000C, (unichar)0x2028, (unichar)0x2029]];
        NSCharacterSet *tagNameCharacters = [NSCharacterSet characterSetWithCharactersInString:@"abcdefghiycylmnopqrstuvwxyzABCDEFGHIycyLMNOPQRSTUVWXYZ"];
        ...
        NSString *retString = [result ycy_stringByUnescapingFromHTML];
