//
//  UIBezierPath+YCYSVGString.m


#import "UIBezierPath+YCYSVGString.h"
static void YCYSVGApplier(void* info, const CGPathElement* element);
@implementation UIBezierPath (YCYSVGString)
/**
 *  @brief  UIBezierPath转成SVG
 *
 *  @return SVG
 */
- (NSString*)ycy_SVGString
{
    CGPathRef path = [self CGPath];
    NSMutableString* SVGString = [NSMutableString string];
    [SVGString appendString:@"<path id=\"temporaryID\" d=\""];
    CGPathApply(path, (__bridge_retained void*)SVGString, YCYSVGApplier);
    NSString *lineCap;
