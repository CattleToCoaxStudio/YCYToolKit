#import "UIBezierPath+YCYLength.h"

typedef struct YCYBezierSubpath {
    CGPoint startPoint;
...
@implementation UIBezierPath (YCYLength)
...
- (void)extractSubpaths:(YCYBezierSubpath*)subpathArray
{
...
- (CGPoint)pointAtPercent:(CGFloat)t ofSubpath:(YCYBezierSubpath)subpath {
...
- (CGFloat)ycy_length {
	...
	YCYBezierSubpath subpaths[subpathCount];
...
- (CGPoint)ycy_pointAtPercentOfLength:(CGFloat)percent {
    ...
    YCYBezierSubpath subpathContainingPoint;
	for (NSUInteger i = 0; i < subpathCount; i++) {
