...
#import "UIView+YCYFind.h"

@implementation UIView (YCYFind)
/**
 *  @brief  找到指定类名的view对象
 *
 *  @param clazz view类名
...
- (id)ycy_findSuperViewWithSuperViewClass:(Class)clazz
{
    if (self == nil) {
        ...
        return [self.superview ycy_findSuperViewWithSuperViewClass:clazz];
    }
}
/**
...
- (BOOL)ycy_findAndResignFirstResponder {
    ...
    for (UIView *v in self.subviews) {
        if ([v ycy_findAndResignFirstResponder]) {
            return YES;
        }
    }
...
- (UIView *)ycy_findFirstResponder {
    ...
    for (UIView *v in self.subviews) {
        UIView *fv = [v ycy_findFirstResponder];
        if (fv) {
            return fv;
        }
    }
 ...
 */
- (UIViewController *)ycy_viewController
{
