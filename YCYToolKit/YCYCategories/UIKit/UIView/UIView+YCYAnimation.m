...
@implementation UIView (YCYAnimation)

#pragma mark - Moves

- (void)ycy_moveTo:(CGPoint)destination duration:(float)secs option:(UIViewAnimationOptions)option {
    [self ycy_moveTo:destination duration:secs option:option delegate:nil callback:nil];
}
...
- (void)ycy_raceTo:(CGPoint)destination withSnapBack:(BOOL)withSnapBack {
    [self ycy_raceTo:destination withSnapBack:withSnapBack delegate:nil callback:nil];
}
...
- (void)ycy_rotate:(int)degrees secs:(float)secs delegate:(id)delegate callback:(SEL)method {
    [UIView animateWithDuration:secs 
                        ...
                        options:UIViewAnimationOptionCurveLinear
                     animations:^{
                         self.transform = CGAffineTransformRotate(self.transform, ycy_radiansForDegrees(degrees));
                     }
                     completion:^(BOOL finished) { 
                         if (delegate != nil) {
#pragma clang diagnostic push
...
- (void)ycy_spinClockwise:(float)secs {
    [UIView animateWithDuration:secs/4 
                        ...
                        options:UIViewAnimationOptionCurveLinear
                     animations:^{
                         self.transform = CGAffineTransformRotate(self.transform, ycy_radiansForDegrees(90));
                     }
                     completion:^(BOOL finished) { 
                         [self ycy_spinClockwise:secs];
                     }];
}
...
- (void)ycy_spinCounterClockwise:(float)secs {
    [UIView animateWithDuration:secs/4 
                        ...
                        options:UIViewAnimationOptionCurveLinear
                     animations:^{
                         self.transform = CGAffineTransformRotate(self.transform, ycy_radiansForDegrees(270));
                     }
                     completion:^(BOOL finished) { 
                         [self ycy_spinCounterClockwise:secs];
                     }];
}
...
- (void)ycy_drainAway:(float)secs {
    self.tag = 20;
	[NSTimer scheduledTimerWithTimeInterval:secs/50 target:self selector:@selector(ycy_drainTimer:) userInfo:nil repeats:YES];
}

- (void)ycy_drainTimer:(NSTimer*)timer {
    ...
    [UIView animateWithDuration:secs/2 
                     ...
                     completion:^(BOOL finished) { 
                         [UIView animateWithDuration:secs/2 
                                          ...
                                          completion:^(BOOL finished) { 
                                              if (continuously) {
                                                  [self ycy_pulse:secs continuously:continuously];
                                              }
                                          }];
                     }];
}
