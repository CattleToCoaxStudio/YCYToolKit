...
#import "UIImageView+YCYFaceAwareFill.h"
#import <CoreImage/CoreImage.h>
#import <QuartzCore/QuartzCore.h>


static CIDetector * _ycy_faceDetector;


@implementation UIImageView (YCYFaceAwareFill)

+ (void)initialize
{
    _ycy_faceDetector = [CIDetector detectorOfType:CIDetectorTypeFace
                                       context:nil
...
- (void)ycy_faceAwareFill {
    // Safe check!
    if (self.image == nil) {
        return;
    }
    
    CGRect facesRect = [self ycy_rectWithFaces];
    if (facesRect.size.height + facesRect.size.width == 0)
        return;
    self.contentMode = UIViewContentModeTopLeft;
    [self ycy_scaleImageFocusingOnRect:facesRect];
}

- (CGRect) ycy_rectWithFaces {
    // Get a CIIImage
    ...
    CIDetector* detector = _ycy_faceDetector;
...
- (void) ycy_scaleImageFocusingOnRect:(CGRect) facesRect {
