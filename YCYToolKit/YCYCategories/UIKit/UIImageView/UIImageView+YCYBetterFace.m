...
#import "UIImageView+YCYBetterFace.h"
...
static CIDetector *ycy_detector;

@implementation UIImageView (YCYBetterFace)

void ycy_hack_uiimageview_bf(){
    Method oriSetImgMethod = class_getInstanceMethod([UIImageView class], @selector(setImage:));
    Method newSetImgMethod = class_getInstanceMethod([UIImageView class], @selector(ycy_setBetterFaceImage:));
    method_exchangeImplementations(newSetImgMethod, oriSetImgMethod);
}

- (void)ycy_setBetterFaceImage:(UIImage *)image{
    [self setYcy_needsBetterFace:image];
    if (![self ycy_needsBetterFace]) {
        return;
    }
    
    [self ycy_faceDetect:image];
}

char nbfKey;
- (void)setYcy_needsBetterFace:(BOOL)needsBetterFace{
...
char detectorKey;
- (void)setYcy_detector:(CIDetector *)detector{
    objc_setAssociatedObject(self,
                             &detectorKey,
                             detector,
                             OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)ycy_faceDetect:(UIImage *)aImage
{
    ...
    dispatch_async(queue, ^{
        ...
        if (ycy_detector == nil) {
            NSDictionary  *opts = [NSDictionary dictionaryWithObject:[self ycy_fast] ? CIDetectorAccuracyLow : CIDetectorAccuracyHigh
                                                              forKey:CIDetectorAccuracy];
            ycy_detector = [CIDetector detectorOfType:CIDetectorTypeFace
                                                      context:nil
                                                      options:opts];
        }
        
        NSArray* features = [ycy_detector featuresInImage:image];
        
        if ([features count] == 0) {
            BFLog(@"no faces");
            dispatch_async(dispatch_get_main_queue(), ^{
                [[self ycy_imageLayer] removeFromSuperlayer];
            });
        } else {
            BFLog(@"succeed %lu faces", (unsigned long)[features count]);
            [self ycy_markAfterFaceDetect:features
...
-(void)ycy_markAfterFaceDetect:(NSArray *)features size:(CGSize)size{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        CALayer *layer = [self ycy_imageLayer];
