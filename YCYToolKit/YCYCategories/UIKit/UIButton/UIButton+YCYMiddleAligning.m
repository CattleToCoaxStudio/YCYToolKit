...
#import "UIButton+YCYMiddleAligning.h"
...
- (UIImage *)ycy_MiddleAlignedButtonImageScaleToSize:(CGSize)size
{
...
@implementation UIButton (YCYMiddleAligning)
...
- (void)ycy_middleAlignButtonWithSpacing:(CGFloat)spacing
{
    ...
    if (imageSize.width > ceilf(maxImageWidth)) {
        ...
        newImage = [self.imageView.image ycy_MiddleAlignedButtonImageScaleToSize:CGSizeMake(maxImageWidth, imageSize.height * ratio)];
        imageSize = newImage.size;
    }
    if (imageSize.height > ceilf(maxImageHeight)) {
        CGFloat ratio = maxImageHeight / imageSize.height;
        newImage = [self.imageView.image ycy_MiddleAlignedButtonImageScaleToSize:CGSizeMake(imageSize.width * ratio, maxImageHeight)];
        imageSize = newImage.size;
    }
    if (newImage) {
