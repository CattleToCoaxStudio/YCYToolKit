...
#import "UIFont+YCYCustomLoader.h"
...
@implementation UIFont (YCYCustomLoader)
 ...
 */
+ (BOOL) ycy_deviceHasFullSupportForFontCollections {
 ...
 */
+ (BOOL)ycy_registerFromURL:(NSURL *)fontURL {
...
/**
 ...
 @return Registration result
 */
+ (BOOL)ycy_registerFromCGFont:(CGFontRef)fontRef {
...
+ (NSArray *)ycy_registerFontFromURL:(NSURL *)fontURL {
    ...
    @synchronized(appRegisteredCustomFonts) {
        ...
        if (fontPSNames == nil) {
            ...
            // Check features
            if ([UIFont ycy_deviceHasFullSupportForFontCollections]) {
                ...
                if (fontDescriptors) {
                    ...
                    if ([fontPSNames count] > 0) {
                        ...
                        if ([UIFont ycy_registerFromURL:fontURL]) {
                    ...
                    if (loadedFont != NULL) {
                        ...
                        if ([singleFontValidExtensions containsObject:[fontURL pathExtension]]) {
                            ...
                            if ([UIFont fontWithName:fontPSNames[0] size:kSizePlaceholder] == nil) {
                                ...
                                // If registration went ok
                                if ([UIFont ycy_registerFromCGFont:loadedFont]) {
                                    // Add url to this library
                                    [appRegisteredCustomFonts setObject:fontPSNames
                                                                 forKey:fontURL];
...
+ (UIFont *) ycy_customFontWithURL:(NSURL *)fontURL size:(CGFloat)size {
    ...
    NSArray *fontPSNames = [UIFont ycy_registerFontFromURL:fontURL];
...
+ (UIFont *) ycy_customFontOfSize:(CGFloat)size withName:(NSString *)name withExtension:(NSString *)extension {
    ...
    return [UIFont ycy_customFontWithURL:fontURL size:size];
}
@end
