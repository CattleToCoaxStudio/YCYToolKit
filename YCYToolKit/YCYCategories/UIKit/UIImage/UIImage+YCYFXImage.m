...
#import "UIImage+YCYFXImage.h"

@implementation UIImage (YCYFXImage)
...
- (UIImage *)ycy_imageScaledToFitSize:(CGSize)size
{
    ...
    {
        return [self ycy_imageScaledToSize:CGSizeMake(size.width, size.width / aspect)];
    }
    else
    {
        return [self ycy_imageScaledToSize:CGSizeMake(size.height * aspect, size.height)];
    }
}
...
- (UIImage *)ycy_reflectedImageWithScale:(CGFloat)scale
{
	...
	//clip to gradient
	CGContextClipToMask(context, bounds, [[self class] ycy_gradientMask]);
...
{
    //get reflected image
    UIImage *reflection = [self ycy_reflectedImageWithScale:scale];
