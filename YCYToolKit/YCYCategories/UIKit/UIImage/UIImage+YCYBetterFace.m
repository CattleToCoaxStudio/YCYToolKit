...
#import "UIImage+YCYBetterFace.h"
...
@implementation UIImage (YCYBetterFace)

- (UIImage *)ycy_betterFaceImageForSize:(CGSize)size
                           accuracy:(YCYAccuracy)accurary;
{
    NSArray *features = [UIImage _ycy_faceFeaturesInImage:self accuracy:accurary];
    ...
    if ([features count]==0) {
        ...
        return [self _ycy_subImageForFaceFeatures:features
                                         size:size];
    }
}

- (UIImage *)_ycy_subImageForFaceFeatures:(NSArray *)faceFeatures size:(CGSize)size
{
...
+ (NSArray *)_ycy_faceFeaturesInImage:(UIImage *)image accuracy:(YCYAccuracy)accurary
{
    CIImage *ciImage = [CIImage imageWithCGImage:image.CGImage];
    NSString *accuraryStr = (accurary == YCYAccuracyLow) ? CIDetectorAccuracyLow : CIDetectorAccuracyHigh;
