...
#import "UIImage+YCYRemoteSize.h"
...
@interface NSURL (YCYRemoteSize)
@property (nonatomic, strong) NSMutableData *ycy_sizeRequestData;
@property (nonatomic, strong) NSString *ycy_sizeRequestType;
@property (nonatomic, copy) YCYUIImageSizeRequestCompleted ycy_sizeRequestCompletion;
@end

@implementation NSURL (RemoteSize)

- (void)setYCY_sizeRequestCompletion: (YCYUIImageSizeRequestCompleted) block {
    objc_setAssociatedObject(self, &kSizeRequestCompletionKey, block, OBJC_ASSOCIATION_COPY);
}

- (YCYUIImageSizeRequestCompleted)ycy_sizeRequestCompletion {
    return objc_getAssociatedObject(self, &kSizeRequestCompletionKey);
}

- (void)setYCY_sizeRequestData:(NSMutableData *)sizeRequestData {
    objc_setAssociatedObject(self, &kSizeRequestDataKey, sizeRequestData, OBJC_ASSOCIATION_RETAIN);
}

- (NSMutableData*)ycy_sizeRequestData {
    return objc_getAssociatedObject(self, &kSizeRequestDataKey);
}

- (void)setYCY_sizeRequestType:(NSString *)sizeRequestType {
    objc_setAssociatedObject(self, &kSizeRequestTypeKey, sizeRequestType, OBJC_ASSOCIATION_RETAIN);
}

- (NSString*)ycy_sizeRequestType {
    return objc_getAssociatedObject(self, &kSizeRequestTypeKey);
}

#pragma mark - NSURLConnectionDelegate
- (void)connection:(NSURLConnection*)connection didReceiveResponse:(NSURLResponse *)response {
    [self.ycy_sizeRequestData setLength: 0];    //Redirected => reset data
}

- (void)connection:(NSURLConnection*)connection didReceiveData:(NSData *)data {
    NSMutableData* receivedData = self.ycy_sizeRequestData;
    
    if( !receivedData ) {
        receivedData = [NSMutableData data];
        self.ycy_sizeRequestData = receivedData;
    }
    ...
    if(!self.ycy_sizeRequestType ) {
        if( memcmp(pngSignature, cString, 8) == 0 ) {
            self.ycy_sizeRequestType = @"PNG";
        }
        else if( memcmp(bmpSignature, cString, 2) == 0 ) {
            self.ycy_sizeRequestType = @"BMP";
        }
        else if( memcmp(jpgSignature, cString, 2) == 0 ) {
            self.ycy_sizeRequestType = @"JPG";
        }
        else if( memcmp(gifSignature, cString, 2) == 0 ) {
            self.ycy_sizeRequestType = @"GIF";
        }
    }
    
    if( [self.ycy_sizeRequestType isEqualToString: @"PNG"] ) {
        char type[5];
        int offset = 8;
        ...
        if( strcmp(type, "IHDR") == 0 ) {   //Should always be first
            ...
            if( self.ycy_sizeRequestCompletion ) {
                self.ycy_sizeRequestCompletion(self, CGSizeMake(width, height));
            }
            
            self.ycy_sizeRequestCompletion = nil;
            
            [connection cancel];
        }
    }
    else if( [self.ycy_sizeRequestType isEqualToString: @"BMP"] ) {
        int offset = 18;
        dword width = 0, height = 0;
        memcpy(&width, cString+offset, 4);
        offset += 4;
    ...
    else if( [self.ycy_sizeRequestType isEqualToString: @"JPG"] ) {
        int offset = 4;
    ...
    else if( [self.ycy_sizeRequestType isEqualToString: @"GIF"] ) {
        int offset = 6;
        dword width = 0, height = 0;
...
-(void)connection:(NSURLConnection*)connection didFailWithError:(NSError *)error {
    if( self.ycy_sizeRequestCompletion )
        self.ycy_sizeRequestCompletion(self, CGSizeZero);
}
...
- (void)connectionDidFinishLoading: (NSURLConnection *)connection {
    ...
    if(!self.ycy_sizeRequestData.length) {
        self.ycy_sizeRequestData = nil;
    }
    else {
        //Try parse to UIImage
        UIImage* image = [UIImage imageWithData: self.ycy_sizeRequestData];
        
        if( self.ycy_sizeRequestCompletion && image) {
            self.ycy_sizeRequestCompletion(self, [image size]);
            return;
        }
    }
...
+ (void)requestSizeNoHeader:(NSURL*)imgURL completion:(YCYUIImageSizeRequestCompleted)completion{
    
    if([imgURL isFileURL] ) {
        //Load from file stream
    }
    else {
        imgURL.ycy_sizeRequestCompletion = completion;
...
+ (void)requestSizeWithHeader:(NSURL*)imgURL completion:(YCYUIImageSizeRequestCompleted)completion{
//        NSURLRequest* request = [NSURLRequest requestWithURL:imgURL];
//
