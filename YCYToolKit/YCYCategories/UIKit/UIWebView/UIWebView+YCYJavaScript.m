...
#import "UIWebView+YCYJavaScript.h"
#import "UIColor+YCYWeb.h"
@implementation UIWebView (YCYJavaScript)
#pragma mark -
...
- (NSArray *)ycy_getImgs
{
    ...
    for (int i = 0; i < [self ycy_nodeCountOfTag:@"img"]; i++) {
...
- (NSArray *)ycy_getOnClicks
{
    ...
    for (int i = 0; i < [self ycy_nodeCountOfTag:@"a"]; i++) {
...
{
    NSString * jsString = [NSString stringWithFormat:@"document.body.style.backgroundColor = '%@'",[color ycy_webColorString]];
...
- (void)ycy_setFontColor:(UIColor *)color withTag:(NSString *)tagName
{
    NSString *jsString = [NSString stringWithFormat:
                          ...
                          nodes[i].style.color = '%@';}", tagName, [color ycy_webColorString]];
    [self stringByEvaluatingJavaScriptFromString:jsString];
}
/// 改变指定标签的字体大小
